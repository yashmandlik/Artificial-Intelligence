...
def axiom_generator_percept_sentence(t, tvec):
    ...
    p = ["Stench", "Breeze", "Glitter", "Bump", "Scream"]
    a = list()
    for index in range(0,5):
        if tvec[index] == True:
            a.append(p[index] + str(t))
        else:
            a.append("~" + p[index] + str(t))
    axiom_str = ' & '.join(a)
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    """
    axiom_str = ''
    surr_locations = [(x-1,y), (x,y+1), (x+1,y), (x,y-1)]
    pit_locations = []
    for i,j in surr_locations:
        if (i >= xmin) and (i <= xmax) and (j >= ymin) and (j <= ymax):
            pit_locations.append(pit_str(i,j))
    axiom_str += breeze_str(x, y)+' <=> '+'('+" | ".join(pit_locations)+" | "+pit_str(x,y)+')'
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    for i,j in surr_locations:
        if (i >= xmin) and (i <= xmax) and (j >= ymin) and (j <= ymax):
            wumpus_locations.append(wumpus_str(i,j))
    
    axiom_str += stench_str(x, y)+' <=> '+'('+" | ".join(wumpus_locations)+" | "+wumpus_str(x,y)+')'
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for x in range(xmin, xmax + 1):
        for y in range(ymin, ymax + 1):
            wumpus_locations.append(wumpus_str(x, y))
    axiom_str += " | ".join(wumpus_locations)
    
    print(axiom_str)
    
    return axiom_str
#axiom_generator_at_least_one_wumpus(xmin=0, xmax=5, ymin=0, ymax=5)
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for l in locations:
        other = []
        for temp_l in locations:
            if temp_l != l:
                other.append(temp_l)
        no_wumpus = []
        for r in other:
            no_wumpus.append("~"+wumpus_str(r[0], r[1]))
        a.append('('+wumpus_str(l[0], l[1])+' >> '+'('+' & '.join(no_wumpus)+'))')
    axiom_str = ' & '.join(a)
    
    print(axiom_str)

    return axiom_str
#axiom_generator_at_most_one_wumpus(xmin=0, xmax=5, ymin=0, ymax=5)
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    for x in range(xmin, xmax + 1):
        for y in range(ymin, ymax + 1):
            if (x,y) != (xi,yi) :
                loc.append('~' + state_loc_str(x,y,t))
    
    axiom_str += state_loc_str(xi, yi, t)+' & ('+' & '.join(loc)+')'
    
    print(axiom_str)

    return axiom_str
#axiom_generator_only_in_one_location(xi = 1, yi = 1, xmin=0, xmax=5, ymin=0, ymax=5, t=0)
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    if heading.lower() == 'north':
        axiom_str += 'HeadingNorth'+ str(t)+' & ('+'~HeadingSouth'+str(t)+' & '+'~HeadingEast'+str(t)+' & '+'~HeadingWest'+str(t)+')'
    if heading.lower() == 'south':
        axiom_str += 'HeadingSouth'+ str(t)+' & ('+'~HeadingNorth'+str(t)+' & '+'~HeadingEast'+str(t)+' & '+'~HeadingWest'+str(t)+')'
    if heading.lower() == 'east':
        axiom_str += 'HeadingEast'+ str(t)+' & ('+'~HeadingNorth'+str(t)+' & '+'~HeadingSouth'+str(t)+' & '+'~HeadingWest'+str(t)+')'
    if heading.lower() == 'west':
        axiom_str += 'HeadingWest'+ str(t)+' & ('+'~HeadingNorth'+str(t)+' & '+'~HeadingEast'+str(t)+' & '+'~HeadingSouth'+str(t)+')'

    print(axiom_str)
    
    return axiom_str
#axiom_generator_only_one_heading(heading = 'north', t = 0)
...
def axiom_generator_location_OK(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = state_OK_str(x, y, t)+' <=> '+'(~'+pit_str(x,y)+' & ('
    axiom_str += '~'+wumpus_str(x, y)+' | ~'+state_wumpus_alive_str(t)+'))'
    
    print(axiom_str)

    return axiom_str

#axiom_generator_location_OK(x=1, y=1, t=0)
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    return axiom_str
#axiom_generator_breeze_percept_and_location_property(x=1, y=1, t=0)
...
def axiom_generator_stench_percept_and_location_property(x, y, t):
    ...
    return axiom_str
#axiom_generator_stench_percept_and_location_property(x=1, y=1, t=0)
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str += state_loc_str(x,y,t+1) + ' <=> ((' + state_loc_str(x,y,t) + ' & ('
    action_list = ['~Forward'+str(t),'Grab'+str(t),'Shoot'+str(t),'TurnLeft'+str(t),'TurnRight'+str(t),'Wait'+str(t),'Bump'+str(t+1)]
    axiom_str += " | ".join(action_list)+'))' + ' | '
    a = []
    if x-1 >= xmin and x-1 <= xmax and y >= ymin and y <= ymax:
            a.append('('+state_loc_str(x-1,y,t) + ' & ('+state_heading_east_str(t)+' & '+action_forward_str(t)+'))')
    #axiom_str2 = ''
    if x >= xmin and x <= xmax and y+1 >= ymin and y+1 <= ymax:
            a.append('('+state_loc_str(x,y+1,t) + ' & ('+state_heading_south_str(t)+' & '+action_forward_str(t)+'))')
    #axiom_str3 = ''
    if x+1 >= xmin and x+1 <= xmax and y >= ymin and y <= ymax:
            a.append('('+state_loc_str(x+1,y,t) + ' & ('+state_heading_west_str(t)+' & '+action_forward_str(t)+'))')
    #axiom_str4 = ''
    if x >= xmin and x <= xmax and y-1 >= ymin and y-1 <= ymax:
            a.append('('+state_loc_str(x,y-1,t) + ' & ('+state_heading_north_str(t)+' & '+action_forward_str(t)+'))')
    ...
    return axiom_str
#axiom_generator_at_location_ssa(t=1, x=1, y=1, xmin=0, xmax=5, ymin=0, ymax=5)
...
def axiom_generator_heading_north_ssa(t):
    ...
    #action_list = ['Forward','Grab','Shoot','Climb','TurnLeft','TurnRight','Wait']
    no_direction = ['Forward'+str(t),'Grab'+str(t),'Shoot'+str(t),'Wait'+str(t)]
    #direction = ['TurnLeft','TurnRight']
    
    axiom_str += "HeadingNorth"+str(t+1) + " <=> ((HeadingNorth" + str(t) + " & (" + " | ".join(no_direction) + ' | Bump' + str(t+1) + '))'
    axiom_str += ' | (HeadingEast' + str(t) + ' & TurnLeft'+str(t) + ') | '
    axiom_str += '(HeadingWest' + str(t) + ' & TurnRight'+str(t) + '))'

    print(axiom_str)
    
    return axiom_str
#axiom_generator_heading_north_ssa(t=0)
...
def axiom_generator_heading_east_ssa(t):
    ...
    no_direction = ['Forward'+str(t),'Grab'+str(t),'Shoot'+str(t),'Wait'+str(t)]
    axiom_str += "HeadingEast"+str(t+1) + " <=> ((HeadingEast" + str(t) + " & (" + " | ".join(no_direction) + ' | Bump' + str(t+1) + '))'
    axiom_str += ' | (HeadingSouth' + str(t) + ' & TurnLeft'+str(t) + ') | '
    axiom_str += '(HeadingNorth' + str(t) + ' & TurnRight'+str(t) + '))'
    
    print(axiom_str)
    return axiom_str
#axiom_generator_heading_east_ssa(t=0)
...
def axiom_generator_heading_south_ssa(t):
    ...
    no_direction = ['Forward'+str(t),'Grab'+str(t),'Shoot'+str(t),'Wait'+str(t)]
    axiom_str += "HeadingSouth"+str(t+1) + " <=> ((HeadingSouth" + str(t)+ " & (" + " | ".join(no_direction) + ' | Bump' + str(t+1) + '))'
    axiom_str += ' | (HeadingWest' + str(t) + ' & TurnLeft'+str(t) + ') | '
    axiom_str += '(HeadingEast' + str(t) + ' & TurnRight'+str(t) + '))'
...
def axiom_generator_heading_west_ssa(t):
    ...
    no_direction = ['Forward'+str(t),'Grab'+str(t),'Shoot'+str(t),'Wait'+str(t)]
    axiom_str += "HeadingWest"+str(t+1) + " <=> ((HeadingWest" + str(t)+ " & (" + " | ".join(no_direction) + ' | Bump' + str(t+1) + '))'
    axiom_str += ' | (HeadingNorth' + str(t) + ' & TurnLeft'+str(t) + ') | '
    axiom_str += '(HeadingSouth' + str(t) + ' & TurnRight'+str(t) + '))'    
    #print(axiom_str)
...
def axiom_generator_heading_only_north(t):
    ...
    axiom_str += state_heading_north_str(t) + ' <=> (~' + state_heading_south_str(t) + ' & ~' + state_heading_west_str(t) + ' & ~' + state_heading_east_str(t) + ')'
...
def axiom_generator_heading_only_west(t):
    ...
    return axiom_str
#axiom_generator_heading_only_west(t=1)
...
#axiom_generator_only_one_action_axioms(t=1)

...
class PlanRouteProblem(search.Problem):
    ...
    def h(self,node):
        ...
        for goal in self.goals:
            l.append(manhattan_distance_with_heading(node.state, goal))
        minimum = min(l)
    ...
    def actions(self, state):
        ...
        if dir == 0 and (x, y+1) in allowed:
            actions = ['Forward', 'TurnLeft', 'TurnRight']
        elif dir == 1 and (x-1, y) in allowed:
            actions = ['Forward', 'TurnLeft', 'TurnRight']
        elif dir == 2 and (x, y-1) in allowed:
            actions = ['Forward', 'TurnLeft', 'TurnRight']
        elif dir == 3 and (x+1, y) in allowed:
            actions = ['Forward', 'TurnLeft', 'TurnRight']
        else:
...
class PlanShotProblem(search.Problem):
    ...
    def h(self,node):
        ...
        for i in self.goals:
            ...
            for j in self.allowed:
                all_x, all_y = j[0], j[1]
                if (all_x == goal_x) or (all_y == goal_y):
                    l.append(j)
        d = []
        for i in l:
    ...
    def goal_test(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        #Goal state if agent is facing the wumpus
        x, y, dir = state[0], state[1], state[2]

        for i in self.goals:
            goal_x, goal_y = i[0], i[1]
            if goal_x == x:
                if y < goal_y and dir == 0:
                    return True
                elif y > goal_y and dir == 2:
                    return True
            if goal_y == y:
                if x<goal_x and dir == 1:
                    return True
                elif x>goal_x and dir == 3:
                    return True
